...
namespace CreatureClimb
{
    ...
    public static class CrispVisuals
    {
        ...
        {
            // Soft AA blurs 2D sprites and text edges.
        ...
        {
            if (screenHeight < 1f)
                screenHeight = PlatformRuntime.ScreenHeight;
        ...
        public static void MakeUiTextCrisp(Text label)
        {
            if (label == null) return;
            // Integer size avoids half-pixel glyph rasterization.
            label.fontSize = Mathf.Max(1, label.fontSize);
            label.alignByGeometry = false;
            label.horizontalOverflow = label.horizontalOverflow;
            // Dynamic font atlas — prefer crisp sampling when glyphs are 1:1.
            ...
            SnapRectToPixels(label.rectTransform);
        }
    }
}
