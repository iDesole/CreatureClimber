...
{
    /// <summary>
    /// Cleans sprite cutout edges: removes light halos, hardens transparency,
    /// fills edge color under transparent pixels (avoids filter fringes), and
    /// trims empty padding.
    /// </summary>
    public sealed class SpriteColorBleedTool : EditorWindow
    {
        const string ToolTitle = "Clean Sprite Edges";

        [SerializeField] List<Texture2D> textures = new List<Texture2D>();
        [SerializeField] DefaultAsset folder;

        // Remove light edge glow
        ...
        // Fill edge color under transparency
        ...
        [MenuItem("Creature Climb/Sprites/Clean Sprite Edges")]
        public static void Open()
        {
            var w = GetWindow<SpriteColorBleedTool>(ToolTitle);
            w.minSize = new Vector2(420, 520);
            w.Show();
        }

        [MenuItem("Assets/Creature Climb/Clean Selected Sprite Edges", false, 1200)]
        public static void ProcessSelectedMenu()
        {
            var paths = CollectSelectedTexturePaths();
            if (paths.Count == 0)
            {
                EditorUtility.DisplayDialog(ToolTitle, "Select one or more PNG/texture assets first.", "OK");
                return;
            }
            ...
            EditorUtility.DisplayDialog(ToolTitle, $"Processed {count} texture(s).", "OK");
        }

        [MenuItem("Assets/Creature Climb/Clean Selected Sprite Edges", true)]
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                "Sprites (especially AI exports) often keep a white or light glow on cutout edges. " +
                "This tool removes that glow, makes transparency crisp, fills edge color under " +
                "transparent pixels so filtered edges stay clean, and trims empty padding.",
                MessageType.Info);
            ...
            EditorGUILayout.LabelField("1. Remove light edge glow", EditorStyles.boldLabel);
            removeWhiteHalo = EditorGUILayout.Toggle("Remove white/light edge glow", removeWhiteHalo);
            using (new EditorGUI.DisabledScope(!removeWhiteHalo))
            {
                haloPasses = EditorGUILayout.IntSlider("Glow remove passes", haloPasses, 1, 6);
                whiteMinChannel = EditorGUILayout.Slider("How light counts as glow", whiteMinChannel, 160f, 250f);
                whiteMaxSaturation = EditorGUILayout.Slider("Max color strength", whiteMaxSaturation, 10f, 80f);
                fringeAlphaCutoff = EditorGUILayout.Slider("Soft-edge alpha cutoff", fringeAlphaCutoff, 1f, 255f);
            }

            EditorGUILayout.Space(6);
            EditorGUILayout.LabelField("2. Transparency", EditorStyles.boldLabel);
            hardenAlpha = EditorGUILayout.Toggle("Make transparency fully on/off", hardenAlpha);
            using (new EditorGUI.DisabledScope(!hardenAlpha))
                alphaThreshold = EditorGUILayout.Slider("Transparency cutoff", alphaThreshold, 1f, 254f);

            EditorGUILayout.Space(6);
            EditorGUILayout.LabelField("3. Edge color fill", EditorStyles.boldLabel);
            colorBleed = EditorGUILayout.Toggle("Fill edge color under transparent pixels", colorBleed);
            using (new EditorGUI.DisabledScope(!colorBleed))
                bleedIterations = EditorGUILayout.IntSlider("Fill passes", bleedIterations, 1, 32);

            EditorGUILayout.Space(6);
            EditorGUILayout.LabelField("4. Crop", EditorStyles.boldLabel);
            trimPadding = EditorGUILayout.Toggle("Trim empty transparent padding", trimPadding);
            using (new EditorGUI.DisabledScope(!trimPadding))
                trimMargin = EditorGUILayout.IntSlider("Keep margin (px)", trimMargin, 0, 16);

            writeAlongside = EditorGUILayout.Toggle("Save as new file (*_clean)", writeAlongside);
            ...
            {
                if (GUILayout.Button("Clean sprites", GUILayout.Height(32)))
                    RunProcess(overwrite: !writeAlongside);

                if (GUILayout.Button("Clean & overwrite originals", GUILayout.Height(24)))
                {
        ...
        void RunProcess(bool overwrite)
        {
            ...
            if (paths.Count == 0)
            {
                EditorUtility.DisplayDialog(ToolTitle, "No textures to process.", "OK");
                return;
            }
            ...
            try
            {
                ...
                {
                    i++;
                    EditorUtility.DisplayProgressBar(ToolTitle, path, (float)i / paths.Count);
            ...
            AssetDatabase.Refresh();
            EditorUtility.DisplayDialog(ToolTitle, $"Done. Processed {ok}/{paths.Count} texture(s).", "OK");
        }
        ...
        static bool ProcessAssetPath(string assetPath, Settings s, bool overwrite)
        {
            ...
            if (ext != ".png")
            {
                Debug.LogWarning($"[CleanSpriteEdges] Skip non-PNG: {assetPath}");
                return false;
            }

            // Load raw file bytes (not the imported Texture2D) so we keep full
            // fidelity for large sprites without toggling isReadable.
            ...
            if (!tmp.LoadImage(raw, false))
            {
                ...
                Debug.LogError($"[CleanSpriteEdges] Could not decode PNG: {assetPath}");
                return false;
            }
            ...
            Debug.Log($"[CleanSpriteEdges] Wrote {outPath} ({w}x{h})");
            return true;
        }
        ...
        /// <summary>
        /// Transparent pixels inherit the average RGB of neighboring solid pixels
        /// while keeping alpha = 0. Prevents light/dark fringes when the engine
        /// filters or extrudes sprite edges.
        /// </summary>
